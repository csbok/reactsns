...
articleController.serverSide = function(req,res, callback) {
	...
	mysql.pool.getConnection(function (err, conn) {
		...
		query = 'select article_no, content, article.user_no, (select count(1) from good where good_article_no=article_no) as good_count, display_name as author from article, user where article.user_no = user.user_no ORDER BY article_no DESC limit ?';
...
articleController.newArticle = function(req,res) {
	...
	mysql.pool.getConnection(function (err, conn) {
		...
		if (user && user.user_no) {
			if (last_no > 0) {
				query = 'select article_no, content, article.user_no, (select count(1) from follow where leader_user_no=user.user_no and lover_user_no=?) as follow_already, (select count(1) from good where good_article_no = article_no and good_user_no = ?) as good_already, (select count(1) from good where good_article_no=article_no) as good_count, display_name as author from article, user where article.user_no = user.user_no AND article_no < ? ORDER BY article_no DESC limit ?';
			...
			if (last_no > 0) {
				query = 'select article_no, content, article.user_no, (select count(1) from good where good_article_no=article_no) as good_count, display_name as author from article, user where article.user_no = user.user_no AND article_no < ? ORDER BY article_no DESC limit ?';
...
articleController.userArticle = function(req, res) {
	...
	mysql.pool.getConnection(function (err, conn) {
		...
		if (user && user.user_no) {
			conn.query('select article_no, content, article.user_no, (select count(1) from follow where leader_user_no=user.user_no and lover_user_no=?) as follow_already, (select count(1) from good where good_article_no = article_no and good_user_no = ?) as good_already, (select count(1) from good where good_article_no=article_no) as good_count, display_name as author from article, user WHERE user.user_no = article.user_no AND article.user_no = ? ORDER BY article_no DESC', [user.user_no, user.user_no, user_no], function(err, rows) {
				if (err) console.error('err : ' + err);
				console.log('rows : ' + JSON.stringify(rows));
			...
			conn.query('select article_no, content, article.user_no, (select count(1) from good where good_article_no=article_no) as good_count, display_name as author from article, user where user.user_no = article.user_no AND article.user_no = ? ORDER BY article_no DESC', user_no, function(err, rows) {
				if (err) console.error('err : ' + err);
